...
def company_dashboard():
    user_id = session["user_id"]
    this_company = Company.query.filter_by(id=user_id).first()
    if this_company.is_approved == false:
        flash("You are not approved yet")
        return redirect(url_for("login"))
    all_drives = Drive.query.filter_by(company_id=user_id).all()
...
def create_drive():
    user_id = session["user_id"]
    this_user = User.query.filter_by(id=user_id).first()
    if this_user.role != "company" and this_user.is_approved != True:
        flash("You are not authorized to access this page")
        return redirect(url_for("login"))   
    if request.method == "POST":
        name = request.form.get("name")
...
def view_drive(drive_id):
    ...
    if this_user.role != "company" and this_user.is_approved != True and this_user.company_id != this_drive.company_id:
        flash("You are not authorized to access this page")
        return redirect(url_for("login"))
    all_applications = Application.query.filter_by(drive_id=drive_id).all()
    return render_template("view_drive.html", this_drive = this_drive, company_id=company_id, all_applications = all_applications)
...
def student_dashboard():
    user_id = session["user_id"]
    this_student = Student.query.filter_by(id=user_id).first()
    if this_student.is_blacklisted == true:
        flash("You are blacklisted")
        return redirect(url_for("login"))
    all_drives = Drive.query.all()
    return render_template("student_dashboard.html", all_drives = all_drives, student_id=student_id, applied_drives = Application.query.filter_by(student_id=student_id).all())
...
def apply_drive(drive_id):
    ...
    this_user = User.query.filter_by(id=user_id).first()
    this_drive = Drive.query.filter_by(id=drive_id).first()
    if this_user.role != "student" and this_user.is_blacklisted != True and this_drive.status != "Upcoming" and this_user.id == Application.query.filter_by(drive_id=drive_id).filter_by(student_id=user_id).first():
        flash("You are not authorized to access this page")
        return redirect(url_for("login"))
